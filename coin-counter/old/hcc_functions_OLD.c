...
void increment_accumulator(struct matrix *input, struct matrix *accumulator, int n_process, int myrank) {
    ...
    split_matrix.data = (int*) malloc(split_matrix.rows * split_matrix.cols * (split_matrix.faces + tail)* sizeof(int));
    ...
    MPI_Gather(split_matrix.data, split_matrix.rows * split_matrix.cols * split_matrix.faces, MPI_INT,accumulator->data, accumulator->rows * accumulator->cols * accumulator->faces, MPI_INT,0, MPI_COMM_WORLD);
}
